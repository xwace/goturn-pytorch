...
try:
    ...
    logger.error('TRAINED Please run $source settings.sh from root directory')
    sys.exit(1)
...
class GoturnTrain(LightningModule):
    ...
    def __init__(self, hparams, dbg=False):
        ...
        self._model = GoturnNetwork(self.hparams_.pretrained_model)
        self._dbg = dbg
        if dbg:
        ...
        if 1:
            ...
            for layer in regression_layer.modules():
                if type(layer) == torch.nn.modules.linear.Linear:
                    param_dict.append({'params': layer.weight,
                                       'lr': 10 * self.hparams_.lr,
                                       'weight_decay': self.hparams_.wd})
                    param_dict.append({'params': layer.bias,
                                       'lr': 20 * self.hparams_.lr,
                                       'weight_decay': 0})
        return param_dict
    ...
    def find_lr(self):
        ...
        optimizer = CaffeSGD(params,
                             lr=1e-8,
                             momentum=self.hparams_.momentum,
                             weight_decay=self.hparams_.wd)

        lr_finder = LRFinder(model, optimizer, criterion, device="cpu")#cuda
    ...
    def configure_optimizers(self):
        ...
        logger.info('Configuring optimizer: SGD with lr = {}, momentum = {}'.format(self.hparams_.lr, self.hparams_.momentum))
        params = self.__set_lr()
        optimizer = CaffeSGD(params,
                             lr=self.hparams_.lr,
                             momentum=self.hparams_.momentum,
        ...
        scheduler = torch.optim.lr_scheduler.StepLR(optimizer,
                                                    step_size=self.hparams_.lr_step,
                                                    gamma=self.hparams_.gamma)
    ...
    def train_dataloader(self):
        ...
        imagenet_path = self.hparams_.imagenet_path
        alov_path = self.hparams_.alov_path
        mean_file = None
        manager = Manager()
        objGoturn = GoturnDataloader(imagenet_path, alov_path,
                                     ...
                                     val_ratio=0.005,
                                     isTrain=True, dbg=False)#返回imagenet_pair, alov_pair元组
        train_loader = DataLoader(objGoturn,
                                  batch_size=self.hparams_.batch_size, shuffle=True,
                                  num_workers=0,
                                  collate_fn=objGoturn.collate)#collate为重组batch所有x样本到一个list，y样本同理
    ...
    def vis_images(self, prev, curr, gt_bb, pred_bb, prefix='train'):
        ...
        for i in range(0, prev.shape[0]):
            # _mean = np.load(self.hparams_.mean_file)
            ...
            curr_img = draw.bbox(curr_img, gt_bb_i, color=(255, 0, 0))#当前框是红色
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self._dbg:
            if batch_idx % 100 == 0:
                d = {'loss': loss.item()}
                iters = (self.trainer.num_val_batches[0] - 1) * self.current_epoch + batch_idx
...
def main(hparams):
    ...
    # model.find_lr()
    ckpt_cb = ModelCheckpoint(hparams.save_path, save_top_k=-1,save_weights_only=True)
    trainer = Trainer(max_epochs=6, gpus=0, callbacks=[ckpt_cb])
    # trainer = Trainer(limit_train_batches=0.1, limit_val_batches=0.1, limit_test_batches=0.1)
    trainer.fit(model)


    #1.直接在fit后面用torch.onnx输出模型model._model
    # torch.onnx.export(model._model,(torch.rand(1,3,227,227),torch.rand(1,3,227,227)),  # model input (or a tuple for multiple inputs)
    #                   "oswin.onnx",export_params=True, opset_version=10,do_constant_folding=True,
    #                   input_names=['x1', 'x2'],
    #                   output_names=['out']
    #                   )
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        #精简onnx模型
        # onnx_model = onnx.load("oswin.onnx")  # load onnx model
        # model_simp, check = simplify(onnx_model)
        # assert check, "Simplified ONNX model could not be validated"
        # onnx.save(model_simp, "oswin.onnx")
        # print("finish")

        #pytorch方式推理
        # x1 = torch.rand(1, 3, 227,227) *255
        # x2 = torch.rand(1, 3, 227,227) *255
        # out = model(x1, x2)
        # print(out)
        ...
        #     "x1": x1.numpy().astype(np.float32),
        #     "x2": x2.numpy().astype(np.float32)
        # })
        #
        # print(x1,"\n",x2)
        # print("\n",output)
