...
try:
    ...
    logger.error('IMAGENET Please run $source settings.sh from root directory')
    sys.exit(1)
...
class ImageNetDataset:
    ...
    def __init__(self, imgs_dir, ann_dir, isTrain=True, val_ratio=0.2, dbg=False):
        ...
        #imgs_dir = "/home/star/Desktop/goturn-pytorch/datasets/ImageNet/images"
        self._imgs_dir = Path(imgs_dir)#封装路径为path对象，调用iterdir函数迭代该目录下所有文件
    ...
    def __loadImageNetDet(self, isTrain=True, val_ratio=0.2):
        ...
        #目录 x = “/home/star/Desktop/goturn-pytorch/datasets/ImageNet/images/n01495701”
        subdirs = [x.parts[-1] for x in self._imgs_dir.iterdir() if x.is_dir()]#x.parts[-1] == n01495701
        train_ratio = 1 - val_ratio
        num_end = int(train_ratio * len(subdirs))#拆分训练要用的文件个数，比如10个文件里面训练用0.6*10 = 6个
        ...
        for i, subdir in enumerate(subdirs):
            ann_files = self._ann_dir.joinpath(subdir).glob('*.xml')#ann_files==所有标注文件名**.xml
    ...
    def __load_annotation(self, idx):
        ...
        img_path = self._imgs_dir.joinpath(random_ann.image_path.with_suffix('.JPEG'))#_imgs_dir+n01443537/n01443537_3817.JPEG
